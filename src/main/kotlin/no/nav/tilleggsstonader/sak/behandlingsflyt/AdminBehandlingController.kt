...
class AdminBehandlingController(
    ...
    fun ferdigstillBehandling() {
        val behandlingId = BehandlingId.fromString("4ed927ed-6d69-42b5-9391-137dd7c94962")
    ...
    fun ferdigstill(behandlingId: BehandlingId) {
        transactionHandler.runInNewTransaction {
            ...
            if (behandling.steg == StegType.JOURNALFØR_OG_DISTRIBUER_VEDTAKSBREV) {
                behandlingService.oppdaterStegPåBehandling(behandling.id, StegType.FERDIGSTILLE_BEHANDLING)
                stegService.håndterSteg(behandling.id, ferdigstillBehandlingSteg)
            }
        }
    }
}
