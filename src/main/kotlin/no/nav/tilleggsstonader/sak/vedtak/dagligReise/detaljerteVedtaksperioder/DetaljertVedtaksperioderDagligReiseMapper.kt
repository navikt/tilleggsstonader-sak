...
object DetaljertVedtaksperioderDagligReiseMapper {
    ...
    private fun List<BeregningsresultatForPeriode>.tilDetaljertVedtaksperiode(
        ...
        flatMap { periode ->
            periode.grunnlag.vedtaksperioder
                .groupBy { Triple(it.målgruppe, it.typeAktivitet, it.aktivitet) }
                .map { (nøkkel, vedtaksperioder) ->
                    DetaljertVedtaksperiodeDagligReise(
                        fom = vedtaksperioder.minOf { it.fom },
                        tom = vedtaksperioder.maxOf { it.tom },
                        aktivitet = nøkkel.third,
                        typeAktivtet = nøkkel.second,
                        målgruppe = nøkkel.first,
