...
import no.nav.tilleggsstonader.sak.vedtak.barnetilsyn.beregning.TilsynBarnBeregningServiceV2
...
class BeregningV2StepDefinition {
        ...
        try {
            beregningsresultat =
                beregningService.beregn(
                    vedtaksperioderDto = vedtaksperioder,
    ...
    fun `forvent følgende vedtaksperiodeGrunnlag`(
        ...
        perioder.forEachIndexed { index, resultat ->
            ...
            try {
                assertThat(resultat.stønadsperiode.fom).`as` { "fom" }.isEqualTo(forventetResultat.fom)
                assertThat(resultat.stønadsperiode.tom).`as` { "tom" }.isEqualTo(forventetResultat.tom)
