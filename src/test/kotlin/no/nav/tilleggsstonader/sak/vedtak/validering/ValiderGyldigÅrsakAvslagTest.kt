...
class ValiderGyldigÅrsakAvslagTest {
    private val vilkårperiodeServiceMock = mockk<VilkårperiodeService>()
    private val vilkårServiceMock = mockk<VilkårService>()
    private val validerGyldigÅrsakAvslag =
        ValiderGyldigÅrsakAvslag(
            vilkårperiodeService = vilkårperiodeServiceMock,
            vilkårService = vilkårServiceMock,
            unleashService =
                mockk {
                    every { isEnabled(Toggle.SKAL_VALIDERE_ÅRSAK_TIL_AVSLAG) } returns false
                },
        )
    ...
    // TODO: Kommenter inn tester når validering av grunner til avslag aktiveres
//    @Nested
...
//    }
    ...
    private fun mockHentVilkårperioder(
        ...
        every { vilkårperiodeServiceMock.hentVilkårperioder(behandlingId) } returns
            Vilkårperioder(
    ...
    private fun mockHentVilkår(vilkår: List<Vilkår> = emptyList()) {
        every { vilkårServiceMock.hentVilkår(behandlingId) } returns vilkår
    }
}
