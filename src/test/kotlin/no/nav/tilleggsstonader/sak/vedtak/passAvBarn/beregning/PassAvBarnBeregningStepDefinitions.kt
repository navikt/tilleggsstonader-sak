...
import no.nav.tilleggsstonader.sak.vedtak.passAvBarn.BarnIdTilTestIdHolder.barnIder
...
class PassAvBarnBeregningStepDefinitions {
    private val logger = LoggerFactory.getLogger(javaClass)
    val vilkårperiodeRepository = VilkårperiodeRepositoryFake()
    val passAvBarnUtgiftService = mockk<PassAvBarnUtgiftService>()
    ...
    val service =
        PassAvBarnBeregningService(
            vilkårperiodeRepository = vilkårperiodeRepository,
            passAvBarnUtgiftService = passAvBarnUtgiftService,
    ...
    var beregningsresultat: BeregningsresultatPassAvBarn? = null
    ...
    init {
        every { repository.findByIdOrThrow(any()) } returns
            innvilgetVedtak(beregningsresultat = BeregningsresultatPassAvBarn(perioder = emptyList()))
    }
    ...
    fun `perioder fra forrige behandling`(dataTable: DataTable) {
        ...
        every { repository.findByIdOrThrow(any()) } returns
            innvilgetVedtak(beregningsresultat = BeregningsresultatPassAvBarn(perioder = perioder))
    }
    ...
    private fun beregn(
        ...
        every { passAvBarnUtgiftService.hentUtgifterTilBeregning(any()) } returns utgifter
