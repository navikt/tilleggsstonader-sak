...
import no.nav.tilleggsstonader.sak.vedtak.passAvBarn.PassAvBarnTestUtil.vedtakBeregningsresultat
...
class VedtakPassAvBarnTest {
    ...
    inner class AvslåttVedtak {
        ...
        fun `2 like avslag skal være like`() {
            val avslagPassAvBarn = AvslagPassAvBarn(listOf(ÅrsakAvslag.INGEN_AKTIVITET), "asd")
            val avslagPassAvBarn2 = AvslagPassAvBarn(listOf(ÅrsakAvslag.INGEN_AKTIVITET), "asd")

            assertThat(avslagPassAvBarn).isEqualTo(avslagPassAvBarn2)
        }
    }
    ...
    inner class OpphørtVedtak {
        ...
        fun `2 like opphør skal være like`() {
            ...
            val avslagTilsynBarn =
                OpphørPassAvBarn(vedtakBeregningsresultat, listOf(ÅrsakOpphør.ENDRING_UTGIFTER), "asd", emptyList(), plan)
